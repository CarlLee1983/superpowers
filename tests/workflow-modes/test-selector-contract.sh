...
if [[ -f "$SELECTOR" ]]; then
  ...
  assert_contains "$SELECTOR" 'Before the first mutation, visibly complete exactly one checkpoint branch:' "selector defines one pre-mutation checkpoint"
...
assert_contains "$BOOTSTRAP" 'Remaining `standard`: output a short visible inline outline covering approach, affected files or components, and verification.' "bootstrap requires standard outline before mutation"
