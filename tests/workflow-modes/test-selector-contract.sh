...
if [[ -f "$SELECTOR" ]]; then
  ...
  assert_contains "$SELECTOR" 'For `rg --files`, allow only repeated `-g <glob>` or `--glob <glob>` pairs followed by project path operands.' "selector allowlists rg files discovery options"
  assert_contains "$SELECTOR" 'For `rg -n` or `rg --line-number`, allow only repeated `-g <glob>` or `--glob <glob>` pairs plus `--hidden`, `--no-heading`, and `--color=never` before the first positional.' "selector allowlists rg content discovery options"
  assert_contains "$SELECTOR" 'The first positional is the search pattern; all remaining positionals are project path operands.' "selector closes rg content discovery operands"
