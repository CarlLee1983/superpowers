...
if [[ -f "$SELECTOR" ]]; then
  ...
  assert_contains "$SELECTOR" 'The only permitted discovery pipeline is one `rg --files` or `rg -n` command followed by one output-limiting `sed -n` command.' "selector closes the read-only discovery pipeline grammar"
